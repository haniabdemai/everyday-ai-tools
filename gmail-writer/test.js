#!/usr/bin/env node
/**
 * Test suite for gmail-writer MCP server.
 *
 * Tests each tool directly via the Gmail API (same code path as the MCP server)
 * plus token validity and error handling. The suite is non-destructive rather
 * than read-only: test 5 stars a real thread and immediately unstars it,
...
// ── Test 6: Error handling with a bad thread ID ──
...
async function testServerLoads() {
  ...
  try {
    // Just check the file exists and is readable; actually starting the
    // server would block on stdio.
...
// ── Test 9: Verify Claude Desktop config ──
async function testClaudeDesktopConfig() {
  ...
  if (!existsSync(configPath))
    return skip(name, "claude_desktop_config.json not found (Claude Desktop not set up on this machine)");
  try {
    ...
    if (!entry.args?.[0]?.includes("gmail-writer/index.js"))
      return fail(name, `args don't point to gmail-writer/index.js`);
    ok(name);
  } catch (e) {
    fail(name, e.message);
  }
...
// ── Test 10: Verify Claude Code config ──
async function testClaudeCodeConfig() {
  const name = "Claude Code config has gmail-writer entry";
  // gmail-writer may be registered in either location.
  const candidates = [
    {
      path: join(homedir(), ".claude/mcp_servers.json"),
      extract: (config) => config["gmail-writer"],
    },
    {
      path: join(homedir(), ".claude.json"),
      extract: (config) => config.mcpServers?.["gmail-writer"],
    },
  ];
  const present = candidates.filter((c) => existsSync(c.path));
  if (present.length === 0)
    return skip(name, "no Claude Code config found (~/.claude/mcp_servers.json or ~/.claude.json)");
  try {
    for (const c of present) {
      const config = JSON.parse(await readFile(c.path, "utf8"));
      const entry = c.extract(config);
      if (!entry) continue;
      const cmd = Array.isArray(entry.command)
        ? entry.command.join(" ")
        : String(entry.command || "");
      if (!cmd.includes("node"))
        return fail(name, `command doesn't include node (${c.path})`);
      return ok(`${name} (${c.path})`);
    }
    fail(name, "no gmail-writer entry in any Claude Code config file");
  } catch (e) {
    fail(name, e.message);
  }
}
